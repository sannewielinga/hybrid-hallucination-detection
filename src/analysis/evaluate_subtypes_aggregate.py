...
def load_and_tag_run_data(csv_path, run_id):
    """Loads data from CSV and tags with run ID."""
    try:
...
def normalize_scores_within_runs(df, score_columns):
    """Applies Z-score normalization within each run."""
...
def perform_aggregate_subtype_analysis(
    ...
    """Analyzes aggregated Z-scores against hallucination types/subtypes."""
    ...
    logging.info(f"Using {num_annotated} valid annotated rows for {dataset_name} aggregate analysis.")
    if num_annotated == 0:
    ...
    def run_mw_test(group1_name, group2_name, justification, data_df, score_col, group_by_col):
        """Helper function to run and print Mann-Whitney U test results."""
            ...
            if (n1 > 1 and n2 > 1 and not np.isclose(var1, 0) and not np.isclose(var2, 0)):
                try:
                    stat, p_value = stats.mannwhitneyu(
                        scores_group1, scores_group2, alternative="two-sided", nan_policy='omit'
                    )
                    ...
                    print(f"(Data lengths: n1={n1}, n2={n2}. Data variances: var1={var1:.2E}, var2={var2:.2E})")
            else:
                print(f"Skipping test: Insufficient samples or zero variance. "
                      f"'{group1_name}': n={n1}, var={var1:.2E}; '{group2_name}': n={n2}, var={var2:.2E}.")
        else:
            missing = []
            if not group1_exists: missing.append(group1_name)
            if not group2_exists: missing.append(group2_name)
            print(f"Skipping test: One or both groups ({', '.join(missing)}) not found in filtered data (n >= {min_samples_per_subtype}).")

    run_mw_test("Factuality", "Faithfulness", "Comparing broad primary types",
                df_annotated, score_column_normalized, group_by_col='hallucination_type')

    run_mw_test("A3_DiagnosticCriteriaDefinition", "B1_ExtrapolationAddition", "Comparing most frequent Factuality vs Faithfulness subtype",
                df_plot_subtype, score_column_normalized, group_by_col='hallucination_subtype')
    run_mw_test("A3_DiagnosticCriteriaDefinition", "A2_ContraindicationIndication", "Comparing conceptual factual errors vs. specific indication errors",
                df_plot_subtype, score_column_normalized, group_by_col='hallucination_subtype')
    run_mw_test("A3_DiagnosticCriteriaDefinition", "A1_ContextIgnorant", "Comparing conceptual factual errors vs. context ignorance errors",
                df_plot_subtype, score_column_normalized, group_by_col='hallucination_subtype')
...
def main():
    ...
    parser.add_argument("--min_samples", type=int, default=3, help="Minimum samples required per subtype for detailed aggregate plotting and stats.")
    ...
    score_cols_to_normalize = [
        ...
        'hybrid_simple_score', 'hybrid_meta_score'
    ]
    ...
    for dataset_name, run_ids in datasets_to_process.items():
        ...
        required_analysis_cols = score_cols_to_normalize + ['hallucination_type', 'hallucination_subtype', 'run_id']
        ...
        df_normalized = normalize_scores_within_runs(df_aggregated, score_cols_to_normalize)

        analysis_cols_normalized = [f"{col}_z" for col in score_cols_to_normalize if f"{col}_z" in df_normalized.columns]
