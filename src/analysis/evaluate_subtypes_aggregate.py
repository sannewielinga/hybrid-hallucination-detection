...
def load_and_tag_run_data(csv_path, run_id):
    """
    Loads a CSV file and assigns a run_id to each row of the DataFrame.
    ...
    run_id : str
        The run_id to assign to each row of the DataFrame
    
    Returns
    -------
    pd.DataFrame or None
        The loaded DataFrame with a 'run_id' column, or None if loading failed
    """
    try:
...
def normalize_scores_within_runs(df, score_columns):
    """
    Applies Z-score normalization to a set of score columns within a DataFrame grouped by a 'run_id' column.

    Parameters
    ----------
    df : pd.DataFrame
        The DataFrame containing the scores to normalize
    score_columns : List[str]
        The names of the score columns to normalize

    Returns
    -------
    pd.DataFrame
        A new DataFrame with the same columns as the input, plus new columns for the normalized scores
    """
...
def perform_aggregate_subtype_analysis(
    ...
    """
    Analyze the distribution of a normalized uncertainty score across different hallucination types and subtypes in an aggregated dataset, and perform statistical tests to determine whether the score is significantly different between the types and subtypes.

    Parameters
    ----------
    df_aggregated_normalized : pd.DataFrame
        The aggregated DataFrame with a 'run_id' column, containing the normalized uncertainty scores and the corresponding hallucination type and subtype labels
    score_column_normalized : str
        The name of the normalized uncertainty score column in the DataFrame
    dataset_name : str
        The name of the dataset being analyzed
    min_samples_per_subtype : int, optional
        The minimum number of samples required per subtype for plotting and statistical testing (default: 2)
    plot_dir : str or Path, optional
        The directory where plots should be saved (default: None)
    ...
    ax_type.set_ylabel(f"Normalized Score ({score_column_original}_z)")
    ax_type.grid(axis="y", linestyle="--", alpha=0.7)
    if save_plots and plot_dir:
        save_plot(fig_type, plot_dir / f"Agg_{dataset_name}_{score_column_original}_vs_Type.png")
    else:
        plt.close(fig_type)
