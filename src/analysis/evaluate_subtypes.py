...
def save_plot(figure, filepath):
    """
    Saves the provided matplotlib figure to the specified file path and logs the result.
    
    Parameters:
        figure (matplotlib.figure.Figure): The figure to be saved.
        filepath (str or Path): The location where the figure will be saved.
    
    Logs:
        Success message if the figure is saved successfully.
        Error message if an exception occurs during saving.
    
    Closes the figure to release resources after attempting to save.
    """
...
def perform_subtype_analysis(
    ...
    """
    Analyze the distribution of a given uncertainty score column across different hallucination types and subtypes in the provided annotated DataFrame, and perform statistical tests to determine whether the score is significantly different between the types and subtypes. The analysis is performed on the subset of data that has valid annotations (i.e., not NaN/unknown).

    For each score column, the function performs the following steps:

    1. Prints basic summary statistics for the score column.
    2. Plots a boxplot of the score distribution by primary hallucination type.
    3. Calculates the AURAC score for the score column on the annotated subset.
    4. Plots a rejection accuracy curve showing the relationship between the fraction of samples rejected and accuracy of the accepted samples.
    5. Prints summary statistics for the score column by hallucination subtype (with at least min_samples_per_subtype samples).
    6. Performs Mann-Whitney U tests to compare the score distributions between the two most frequent primary hallucination categories (Factuality and Faithfulness) and the two most frequent subtypes (A3_DiagnosticCriteriaDefinition and B1_ExtrapolationAddition).

    If the `save_plots` argument is True, the function will save the plots to the provided directory. If `plot_dir` is None, the plots are not saved.
    """
    logging.info(
...
def main():
    """
    Analyze the relationship between hallucination subtypes and uncertainty scores.

    Command-line interface:

    python evaluate_subtypes.py <input_csv> <run_id> [options]

    Options:

        --score_columns <list>    List of score columns to analyze (default: semantic_entropy)
        --min_samples <int>       Minimum samples required per subtype for detailed plotting and stats (default: 2)
        --plot_dir <path>         Directory to save output plots (default: ./analysis_plots)
        --save_plots              Save plots to files instead of displaying them

    The script will load the input CSV file, filter out rows with incomplete annotations, and then perform the subtype
    analysis for each provided score column. Plots will be saved to the specified directory if --save_plots is set.
    """
