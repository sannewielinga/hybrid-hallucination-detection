...
def compatible_bootstrap_aurac(func, rng):
    """
    Converts a function to be compatible with bootstrapping for AURAC performance evaluation.

    This function wraps the input arrays `y_true_correct` and `y_score` into a format suitable for
    bootstrapping and applies the specified `func` to compute metrics like AURAC.

    Parameters
    ----------
    func : callable
        The function to compute a performance metric using `y_true_correct` and `y_score`.
    rng : np.random.Generator
        A NumPy random number generator for reproducibility of the bootstrap sampling.
    ...
    -------
    callable
        A function that takes `y_true_correct` and `y_score` arrays, wraps them for bootstrapping,
        and returns the bootstrapped evaluation metric.
    """
