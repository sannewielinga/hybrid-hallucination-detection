...
def init_model(args):
    ...
    probe_layers_to_extract_val = getattr(args, 'probe_layers_to_extract', [-1, -2, -4])
    if not isinstance(probe_layers_to_extract_val, list):
        logging.warning(f"probe_layers_to_extract in args was not a list ({probe_layers_to_extract_val}), using default [-1, -2, -4].")
        probe_layers_to_extract_val = [-1, -2, -4]
    
    logging.info(f"Initializing HuggingfaceModel with probe_layers_to_extract: {probe_layers_to_extract_val}")
    ...
    if (
        ...
        if args.base_model is not None:
            model = HuggingfaceModel(
                ...
                probe_layers_to_extract=probe_layers_to_extract_val
            )
        else:
            model = HuggingfaceModel(
                mn, 
...
def get_metric(metric):
    if metric == "squad":
        ...
        logging.info("Using BERTScore metric (F1, rescaled) with default model.")
        def bertscore_metric_func(response, example, *args, **kwargs):
            if not isinstance(response, str):
                logging.warning(f"BERTScore: Response is not a string: {response}. Returning 0.0.")
                return 0.0
            
            ref_texts_list = []
            if "answers" in example and "text" in example["answers"]:
                ref_texts_list = example["answers"]["text"]
            elif "reference" in example and "answers" in example["reference"] and "text" in example["reference"]["answers"]:
                ref_texts_list = example["reference"]["answers"]["text"]
            
            if not ref_texts_list or not all(isinstance(r, str) for r in ref_texts_list):
                if isinstance(ref_texts_list, str) and ref_texts_list:
                    ref_texts_list = [ref_texts_list]
                else:
                    logging.warning(f"BERTScore: Invalid or empty reference texts for example ID {example.get('id', 'Unknown')}. Refs: {ref_texts_list}. Returning 0.0.")
                    return 0.0
            
            ref_texts_list = [r for r in ref_texts_list if r and isinstance(r, str)]
            if not ref_texts_list:
                logging.warning(f"BERTScore: All reference texts are empty or invalid for example ID {example.get('id', 'Unknown')}. Returning 0.0.")
                return 0.0

            predictions = [response]
            references = [ref_texts_list] 

            try:
                P, R, F1 = b_score(predictions, references, lang="en", model_type="microsoft/deberta-xlarge-mnli", rescale_with_baseline=True, verbose=False)
                f1_score_val = F1.item() 
            except Exception as e:
                logging.error(f"BERTScore calculation failed for example ID {example.get('id', 'Unknown')}: {e}", exc_info=True)
                return 0.0

            threshold = 0.5 
            is_correct = 1.0 if f1_score_val >= threshold else 0.0
            return is_correct
        metric = bertscore_metric_func
    elif metric == "llm":
        metric = llm_metric
