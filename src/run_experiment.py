...
def main():
    ...
    if run_flags.get("generate", False):
        stages_executed.append("generate"); gen_args = construct_args(config, gen_args_map); gen_args.extend(["--output_dir", str(run_output_dir)])
    ...
    if run_flags.get("compute_se", False):
        ...
        if gen_output_pkl.exists():
            se_args = construct_args(config, se_args_map); se_args.extend(["--run_dir", str(run_output_dir)])
    ...
    # Stage 3: Probe IS (Train & Predict All)
    ...
    # Stage 4: Train Hybrid Meta & Predict All
    ...
    # Stage 5: Calculate Full Set AUROCs
    ...
    if not success and any(run_flags.get(stage) for stage in ['generate', 'compute_se', 'probe_is', 'train_hybrid_meta', 'prepare_analysis']):
        sys.exit(1)
