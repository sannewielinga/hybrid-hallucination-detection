...
def generate_answers(args):
    ...
    :param args: An argparse.Namespace object containing the configuration for the generation process.
    :return: A dictionary of results, including the generated answers and any computed uncertainty measures.

    This function is responsible for:
        - Initializing the model and dataset
        - Constructing the few-shot prompt
        - Generating answers for the given dataset split
        - Computing accuracy and uncertainty metrics
    ...
    for dataset_split in ["train", "validation"]:
        ...
        for it, index in enumerate(
            ...
            for i in range(num_loops):
                ...
                if i == 0:
                    logging.info(
                        f"ID {example_id}: Low-T Gen='{predicted_answer[:60]}...', Acc={acc:.2f}, Len={response_length}"
                    )
                    ...
                    full_responses.append(
                        (predicted_answer, token_log_likelihoods, embedding, float(acc), response_length)
                    )
            ...
            if dataset_split == "validation" and annotation_file_handle:
                annotation_record = {
                    ...
                    "response_length": most_likely_answer_dict_anno.get("response_length", 0),
                    "hallucination_type": "",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs='+', 
        default=[-1, -2, -4], 
        help="List of relative layer indices (from end) to extract hidden states for IS probe (e.g., -1 -2 -4)."
    )
